...
class Member:
    def __init__(self, r_d, label=None, doc_id=None):
        self._r_d = r_d  # td_idf of document D
        self._label = label
        self._doc_id = doc_id
...
class Kmeans():
    ...
    def load_data(self, data_path):
        def sparse_to_dense(sparse_r_d, vocab_size):
            ...
            indicies_tfidfs = sparse_r_d.split()
            for index_tfidf in indicies_tfidfs:
        ...
        with open("../../Session 1/source/20news-bydate/word_idfs.txt") as f:
    ...
    def random_init(self, seed_value):
        ...
        for i in range(self._num_clusters):
            centroid_index = np.random.randint(0, data_length)
            self._E.append(self._data[centroid_index])
            self._clusters[i]._centroid = self._data[centroid_index]._r_d

    def compute_similarity(self, member, centroid):
        return np.sqrt(np.sum((member._r_d - centroid) ** 2))

    # select cluster for a specific document
    def select_cluster_for(self, member):
        best_fit_cluster = None
        max_similarity = -1
        for cluster in self._clusters:
            similatity = self.compute_similarity(member, cluster._centroid)
            if similatity > max_similarity:
                best_fit_cluster = cluster
                max_similarity = similatity
    ...
    def run(self, seed_value, criterion, threshold):
        ...
        self._iterations = 0
        while True:
            # reset clusters, retain only centroid
            for cluster in self._clusters:
